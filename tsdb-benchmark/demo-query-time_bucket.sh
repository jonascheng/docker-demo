...
docker exec -it tsdb psql -U postgres -c "SELECT time_bucket('2 hour', event_time) as two_hours, COUNT(1) FROM sample_events WHERE event_time > now() - INTERVAL '1 days' GROUP BY two_hours ORDER BY two_hours LIMIT 12;"
